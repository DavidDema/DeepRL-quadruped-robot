...
class GOEnv(MujocoEnv):
    ...
    def step(self, delta_q):
        ...
        living_reward = self._reward_living(timestep=self.timestep, max_timesteps=self.max_timesteps, scaling_factor=0, positive=False) # 3.5
        #living_reward = 0
        
        healthy_reward = self._reward_healthy(scaling_factor=5.0) # 1.0
        ...
        z_pos_reward = self._reward_z_pos(after_pos=after_pos, scaling_factor=10.0) # 2.0

...
class GOEnv(MujocoEnv):
    ...
    def _reward_z_vel(self, before_pos, after_pos, before_vel, after_vel,
        ...
        return np.exp(-scaling_factor * z_vel ** 2) # TODO scaling outside exp

    def _reward_z_pos(self, after_pos, scaling_factor=1.0):
        # penalize movement in z direction
        dz = after_pos[2] - self.init_joints[2]*0.5
        return np.exp(-scaling_factor * dz ** 2)

    def _reward_pitch_roll(self, orientation, scaling_factor=1.0):
    ...
    def _reward_yaw(self, yaw, before_pos, after_pos, scaling_factor=1.0):
        ...
        #if -3.0 > yaw_offset > 3.0:
        #    yaw_rew = - np.linalg.norm(yaw - target_yaw)
        #else:
        #    yaw_rew = np.linalg.norm(yaw - target_yaw)

        #return scaling_factor*yaw_rew
        return - scaling_factor * np.linalg.norm(yaw - target_yaw)

    def _reward_yaw_rate(self, before_orientation, after_orientation, scaling_factor=1.0):
    ...
    def _reward_pitch_roll_rate(self, before_orientation, after_orientation, scaling_factor=1.0):
    ...
    def _reward_foot_slip(self, feet_vel, feet_cont, scaling_factor=1.0):  ##(YAN)## sehr viel kleiner machen
    ...
    def _reward_two_feet(self, feet_cont, scaling_factor=1.0):
        ...
        return -scaling_factor * reward_two_feet

    def _reward_going_far(self, now_x, before_x, scaling_factor=1.0):
        way = now_x - before_x
        return scaling_factor * way
    ...
    def step(self, delta_q):
        ...
        z_vel_reward = self._reward_z_vel(before_pos=before_pos, before_vel=before_vel, after_vel=after_vel,
                                          after_pos=after_pos, scaling_factor=2.0)  # 1.0; fkt 2.0
        z_pos_reward = self._reward_z_pos(after_pos=after_pos, scaling_factor=2.0)  # 2.0; fkt 2.0
        foot_slip_reward = self._reward_foot_slip(feet_vel, feet_cont, scaling_factor=0.3)  # fkt 5.0
        two_feet_reward = self._reward_two_feet(feet_cont,
                                                scaling_factor=0.0)  # fkt 5.0 ##(YAN)## should work without, just optional
        reward_way_forward = self._reward_going_far(now_x=now_x, before_x=before_x, scaling_factor=0.0)
        reward_way_sideways = self._reward_going_side(now_y=now_y, before_y=before_y, scaling_factor=0.0)

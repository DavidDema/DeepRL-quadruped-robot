...
class GOEnv(MujocoEnv):
    ...
    def upper_limits(self):
        ...
        return np.array([-0.0, 2.5, -1.6] + [0.2, 2.5, -1.6] + [-0.0, 2.5, -1.0] + [0.2, 2.5, -1.0]) ## von hinten aus gesehen vr, vl, hr, hl
    @property
    def init_joints(self):
        ...
        return np.array([0, 0, 0.37, 1, 0, 0, 0] + [0.1, 0.6, -1.8] + [-0.1, 0.6, -1.8] + [-0.1, 0.6, -1.7] + [0.1, 0.6, -1.7])
    @property
    ...
    def _reward_z_vel(self, before_pos, after_pos, before_vel, after_vel, scaling_factor=5.0): ##(YAN)## should be very small, in total not so important, velocity is a big value
    ...
    def step(self, delta_q):
        ...
        track_vel_reward = self._reward_lin_vel(before_pos, after_pos, before_vel, after_vel, scaling_factor=15.0) # 1.5; fkt 0.5 ##(YAN)## should be the biggest
        living_reward = self._reward_living(timestep=self.timestep, max_timesteps=self.max_timesteps, scaling_factor=0.1, positive=False) # 3.5; fkt 5
        #living_reward = 0
        
        healthy_reward = self._reward_healthy(scaling_factor=5.0) # 1.0; fkt 15 ##(YAN)##increase, damit er nicht einfach vorne über fällt!
        yaw_rate_reward = self._reward_yaw_rate(before_orientation, after_orientation, scaling_factor=0.0) # 0.8; fkt 0.2
        pitchroll_rate_reward = self._reward_pitch_roll_rate(before_orientation, after_orientation, scaling_factor=0.0) # 0.05; fkt 0.2
        pitchroll_reward = self._reward_pitch_roll(after_orientation, scaling_factor=0.0) # 5.0; fkt 2.0
        joint_pos_reward = self._reward_joint_pose(after_joints, self.init_joints[7:], scaling_factor=0.0) # 0.3; fkt 0.3
        orient_reward = self._reward_yaw(after_yaw, before_pos, after_pos, scaling_factor=1.0) # 0.1; fkt 0.1
        z_vel_reward = self._reward_z_vel(before_pos=before_pos, before_vel=before_vel, after_vel=after_vel, after_pos=after_pos, scaling_factor=1.5) # 1.0; fkt 2.0
        ...
        two_feet_reward = self._reward_two_feet(feet_cont, scaling_factor = 1.0) # fkt 5.0 ##(YAN)## should work without, just optional

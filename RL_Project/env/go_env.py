...
class GOEnv(MujocoEnv):
    ...
    def _reward_lin_vel(self, before_pos, after_pos, scaling_factor=10.0):
        ...
        target_vel = np.array([0.2, 0.05, 0.0])
    ...
    def step(self, delta_q):
        ...
        track_vel_reward = self._reward_lin_vel(before_pos, after_pos, scaling_factor=10.0) # 1.5; fkt 0.5 ##(YAN)## should be the biggest
        ...
        yaw_rate_reward = self._reward_yaw_rate(before_orientation, after_orientation, scaling_factor=0.2) # 0.8; fkt 0.2
        pitchroll_rate_reward = self._reward_pitch_roll_rate(before_orientation, after_orientation, scaling_factor=0.2) # 0.05; fkt 0.2
        pitchroll_reward = self._reward_pitch_roll(after_orientation, scaling_factor=0.2) # 5.0; fkt 2.0
        ...
        two_feet_reward = self._reward_two_feet(feet_cont, scaling_factor = 0.1) # fkt 5.0 ##(YAN)## should work without, just optional
